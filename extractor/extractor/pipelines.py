...
class BoiteAGrainesPipeline:
    def process_item(self, item, spider):
        if item['vendor'] != 'laboiteagraine.com':
            return item
        item['price'] = self.parse_price(item['price'])
        item['product_name'] = self.parse_name(item['product_name'])
        item['description'] = remove_tags(item['description']).strip()  # laboiteagraine.com
    ...
    def get_weight(self, description):
        pattern = re.compile(r'\([^\d]*([\d\,\.]+)\s*gramme', re.IGNORECASE)  # laboiteagraine.com
        match = re.search(pattern, description)
        if not match:
            return 0
...
class KokopelliPipeline:
    def process_item(self, item, spider):
        if item['vendor'] != 'kokopelli-semences.fr':
            return item
