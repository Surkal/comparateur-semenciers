...
def test_price_parsing(input, output):
    pipeline = BoiteAGrainesPipeline()
    assert pipeline.parse_price(input) == output


@pytest.mark.parametrize(
    'description,quantity',
    [
        ('On peut les utiliser pour faire des ratatouilles, en beignets, sautées à la poêle. Variété non inscrite (0,2 GRAMME = +/- 50 GRAINES)', 50.0),
        ('On peut les utiliser pour faire des ratatouilles, en beignets, sautées à la poêle. Variété non inscrite (0,2 GRAMME = +/- 50 graines)', 50.0),
        ('On peut les utiliser pour faire des ratatouilles, en beignets, sautées à la poêle. Variété non inscrite (0,2 GRAMME = +/- 50 TUBERCULES)', 50.0),
        ('On peut les utiliser pour faire des ratatouilles, en beignets, sautées à la poêle. Variété non inscrite (0,2 GRAMME = +/- 50 tubercules)', 50.0),
